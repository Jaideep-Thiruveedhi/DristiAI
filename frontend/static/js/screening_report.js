...
function displayScreening(
    ...
    if (gradcamImage) {
        ...
        if (screening.heatmap_base64) {

            gradcamImage.src =
                `data:image/png;base64,${screening.heatmap_base64}`;

            gradcamImage.style.display =
                "block";

            gradcamImage.parentElement.classList.add(
                "has-heatmap"
            );

            if (gradcamFallback) {

                gradcamFallback.style.display =
                    "none";
            }

        } else {

            gradcamImage.removeAttribute(
                "src"
            );
